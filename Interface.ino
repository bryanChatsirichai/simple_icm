...
int calibrate(const char *const string_table[], int upper_limit, int lower_limit, uint16_t color) {
  int pos_current;
...
// callibrate pov
int chooseDist(int count, const char *const string_table[], bool goBack, uint16_t color) {
  int pos_current, upper_limit;
  pos_current = lens_current;
  upper_limit = lens_range;
  ...
  if (goBack) {
    ...
    moveMotor(lens_current, 0);
  }
  
  return pos_current;
}

void goDist(const char title[], int pos_desired, uint16_t color, float motor_time, float motor_div,bool goBack, bool lastSequence,bool showScreen) {
  ...
  switch (excess_option_set){
    ...
    case 0:{
      ...
      moveMotor(pos_desired, motor_time / motor_div);
      break;
    }
    //split
    case 1:{
  ...
  //lens current postion after moving to desired position
  lens_current = pos_desired;
  ...
  if (goBack) {
    if(showScreen){
      printMoveSteps(title, color,1);
    }
    moveMotor(pos_current, 0);
    //lens current postion after moving to desired position
    lens_current = pos_current;
   updateScreen(0);
  }
}
